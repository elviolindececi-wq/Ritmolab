...
const rhythmOptions: RhythmOption[] = [
  ...
  {
    label: "Blancas parejas",
    abc: "X:1\nM:4/4\nL:1/4\nQ:1/4=70\nK:C\n| C2 D2 |",
    syllables: ["TA-aa", "TA-aa"]
  },
  {
    label: "Redonda completa",
    abc: "X:1\nM:4/4\nL:1/4\nQ:1/4=66\nK:C\n| C4 |",
    syllables: ["TAaaaa"]
  },
  {
    label: "Blanca y dos negras",
    abc: "X:1\nM:4/4\nL:1/4\nQ:1/4=74\nK:C\n| C2 D E |",
    syllables: ["TA-aa", "TA", "TA"]
  },
  {
    label: "Negra blanca negra",
    abc: "X:1\nM:4/4\nL:1/4\nQ:1/4=74\nK:C\n| C D2 E |",
    syllables: ["TA", "TA-aa", "TA"]
  },
  {
    label: "Blanca silencio negra",
    abc: "X:1\nM:4/4\nL:1/4\nQ:1/4=72\nK:C\n| C2 z D |",
    syllables: ["TA-aa", "—", "TA"]
  },
  {
    label: "Acentos en 4/4",
    abc: "X:1\nM:4/4\nL:1/4\nQ:1/4=80\nK:C\n| C D E F |",
    syllables: ["FUERTE", "débil", "medio", "débil"]
  },
  {
    label: "6/8 dos grupos",
    abc: "X:1\nM:6/8\nL:1/8\nQ:1/4=70\nK:C\n| C D E F G A |",
    syllables: ["TA-ki-ta", "TA-ki-ta"]
  },
  {
    label: "6/8 con silencio",
    abc: "X:1\nM:6/8\nL:1/8\nQ:1/4=70\nK:C\n| C D z F G A |",
    syllables: ["TA", "ki", "—", "TA", "ki", "ta"]
  },
  {
    label: "6/8 largo-corto",
    abc: "X:1\nM:6/8\nL:1/8\nQ:1/4=70\nK:C\n| C3 D E F |",
    syllables: ["TA-aa-a", "TA", "ki", "ta"]
  },
  {
    label: "Memoria con silencio",
    abc: "X:1\nM:4/4\nL:1/8\nQ:1/4=78\nK:C\n| C2 z2 D E F2 |",
    syllables: ["TA", "—", "TA-KA", "TA"]
  }
];
...
const rhythmByLabel = Object.fromEntries(rhythmOptions.map((option) => [option.label, option])) as Record<string, RhythmOption>;

function option(label: string) {
  const found = rhythmByLabel[label];
  if (!found) throw new Error(`No existe el patrón rítmico: ${label}`);
  return found;
}

function withLessonBpm(pattern: RhythmOption, bpm: number): RhythmOption {
  return {
    ...pattern,
    abc: pattern.abc.replace(/^Q:\s*([^=]+)=\s*\d+/m, `Q:$1=${bpm}`),
  };
}

const lessonDictationLabels: Record<string, string[]> = {
  "pulso-y-negra": ["Negras parejas", "Silencio de negra", "Solo corcheas"],
  "silencios-basicos": ["Silencio de negra", "Silencio al final", "Corchea + silencio"],
  "corcheas": ["Solo corcheas", "Negras parejas", "Negra y corcheas"],
  "blanca-intro": ["Blancas parejas", "Negras parejas", "Redonda completa"],
  "redonda-intro": ["Redonda completa", "Blancas parejas", "Negras parejas"],
  "duraciones-mixtas": ["Blanca y dos negras", "Negra blanca negra", "Blanca silencio negra"],
  "negras-corcheas": ["Negra y corcheas", "Negras parejas", "Solo corcheas"],
  "compas-cuatro-cuatro": ["Acentos en 4/4", "Negras parejas", "Silencio al final"],
  "dictado-simple": ["Negra y corcheas", "Negras parejas", "Silencio de negra"],
  "tresillos-intro": ["Tresillo inicial", "Solo corcheas", "Negra y corcheas"],
  "semicorcheas-intro": ["Semicorcheas", "Solo corcheas", "Tresillo inicial"],
  "sincopa-intro": ["Síncopa básica", "Contratiempo simple", "Negra puntillo + corchea"],
  "compas-seis-ocho": ["6/8 dos grupos", "6/8 con silencio", "6/8 largo-corto"],
  "lectura-con-metronomo": ["Síncopa básica", "Semicorcheas + negra", "Tresillo inicial"],
  "memoria-ritmica-intro": ["Dos tresillos", "Memoria con silencio", "Silencio al final"],
  "call-response-basico": ["Corchea + silencio", "Silencio de negra", "Negras parejas"],
  "call-response-corcheas": ["Solo corcheas", "Negra y corcheas", "Contratiempo simple"],
  "examen-final-ritmico": ["Síncopa básica", "6/8 dos grupos", "Semicorcheas"],
};

function lessonDictationOptions(item: (typeof rawLessons)[number]) {
  const labels = lessonDictationLabels[item.slug] ?? [
    rhythmOptions[item.rhythmIndex % rhythmOptions.length].label,
    rhythmOptions[(item.rhythmIndex + 1) % rhythmOptions.length].label,
    rhythmOptions[(item.rhythmIndex + 2) % rhythmOptions.length].label,
  ];

  return labels.map((label) => withLessonBpm(option(label), item.bpm));
}

function makeLesson(item: (typeof rawLessons)[number]): Lesson {
  const rhythmList = lessonDictationOptions(item);
  const rhythm = rhythmList[0];
  ...
  return {
    ...
    exercises: [
      ...
      },
      {
        ...
        prompt: `Escuchá un patrón de ${item.title}, mirá las partituras y elegí la que corresponde.`,
        bpm: item.bpm,
        rhythm,
        rhythmOptions: rhythmList,
        correctIndex,
        explanation: `Este dictado practica ${item.objective.toLowerCase()} con patrones de ${item.title}.`,
        xp: 20 + item.level * 4
      },
      {
        type: "takadimi",
