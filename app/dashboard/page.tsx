...
import { ArrowRight, BadgeCheck, Flame, Infinity, LogOut, Map, RotateCcw, Share2, Shield, Trophy } from "lucide-react";
...
function HeroCTA({ lesson }: { lesson: { moduleSlug: string; title: string; xp: number; minutes: number } | null }) {
  ...
  return (
    <div className="card bg-gradient-to-br from-brand-500 to-brand-600 text-white">
      <p className="text-sm font-black uppercase opacity-75 tracking-wide">Tu acción principal de hoy</p>
      <h2 className="mt-1 text-3xl font-black leading-tight md:text-4xl">{lesson.title}</h2>
      <p className="mt-2 text-base opacity-85">Continuá acá primero. {lesson.minutes} min · +{lesson.xp} XP</p>
      <Link href={`/modules/${lesson.moduleSlug}`}
        ...
        Continuar ahora <ArrowRight className="ml-2" size={18} />
      </Link>
    </div>
  );
}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_.55fr]">
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Link href="/today" className="card flex items-center justify-between gap-4 transition hover:-translate-y-0.5 hover:shadow-2xl">
              <div>
                <p className="text-xs font-black uppercase text-brand-700">Entrenador diario</p>
                <h3 className="mt-1 text-xl font-black">Qué practicar hoy</h3>
                <p className="mt-1 text-sm font-bold text-zinc-500">Retos, repasos y habilidad foco.</p>
              </div>
              <Infinity className="text-brand-700" size={28} />
            </Link>
            <Link href="/worlds" className="card flex items-center justify-between gap-4 transition hover:-translate-y-0.5 hover:shadow-2xl">
              <div>
                <p className="text-xs font-black uppercase text-brand-700">Ruta viva</p>
                <h3 className="mt-1 text-xl font-black">Mundos generativos</h3>
                <p className="mt-1 text-sm font-bold text-zinc-500">Curso + práctica infinita.</p>
              </div>
              <Map className="text-brand-700" size={28} />
            </Link>
          </div>
