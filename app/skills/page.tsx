...
// In a real implementation, sessions would come from Supabase app_exercise_attempts.
// For now we derive from lesson progress scores as a proxy.
...
export default function SkillsPage() {
  ...
  useEffect(() => {
    loadMyStats().then(d => {
      const sess = progressToSessions(d.progress ?? []);
      setSessions(sess);
  ...
  return (
    <div className="container-page py-10">
      ...
      <p className="mt-3 max-w-2xl text-lg text-zinc-600">
        Completar lecciones suma XP. Dominar habilidades requiere precisión consistente en múltiples sesiones. Esto mide lo segundo.
      </p>
          ...
          {dueReview.length > 0 && (
            <div className="mt-6 rounded-2xl border border-yellow-300 bg-yellow-50 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {dueReview.map(s => (
                  <Link key={s.id} href="/free-practice" className="rounded-full bg-yellow-200 px-3 py-1 text-xs font-black text-yellow-900 hover:bg-yellow-300 transition">
            ...
            <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-4">
              ...
              <div className="flex flex-wrap gap-2">
                {weak.slice(0, 4).map(s => (
                  <Link key={s.id} href={`/free-practice`}
