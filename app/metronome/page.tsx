...
export default function MetronomePage() {
  ...
  const hits = useMemo(() => pattern.abc ? audibleHits(hitsFromAbc(pattern.abc, pattern.defaultNote ?? "C4")) : pattern.hits?.length ? audibleHits(pattern.hits) : audibleHits(hitsFromSyllables(pattern.syllables, pattern.defaultNote ?? "C4")), [pattern]);
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_.7fr]">
        ...
        <div className="space-y-6">
          <div className="card">
            ...
            <button onClick={() => playRhythmPattern(hits, bpm, pattern.defaultNote ?? "C4", { metronome: true, timeSignature: timeSignatureFromAbc(pattern.abc), totalBeats: totalQuarterBeatsFromAbc(pattern.abc), countInBars: 1 })} className="btn-secondary mt-4">
