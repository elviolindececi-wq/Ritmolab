...
import { playNote, playRhythmPattern, hitsFromAbc, hitsFromSyllables, timeSignatureFromAbc, totalQuarterBeatsFromAbc } from "@/lib/rhythm-engine";
...
export function ExerciseCallResponse({ lesson, exercise, index, onResult }: Props) {
  ...
  const targetHits = rhythm?.abc
    ? hitsFromAbc(rhythm.abc, rhythm.defaultNote ?? "C4")
    : rhythm?.hits?.length
    ? rhythm.hits
  ...
  function startCall() {
    ...
    setTaps([]);
    setScore(null);
    playRhythmPattern(targetHits, bpm, rhythm?.defaultNote ?? "C4", { metronome: true, timeSignature: timeSignatureFromAbc(rhythm?.abc), totalBeats: rhythm?.abc ? totalQuarterBeatsFromAbc(rhythm.abc) : undefined, countInBars: 1 });
    setTimeout(() => {
      setPhase("waiting");
      setCountdown(3);
      let c = 3;
